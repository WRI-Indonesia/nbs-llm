 ...
 * Converts user queries + schema documents into executable SQL
 */
export class SQLGenerationAgent extends BaseAgent {
  private openai: OpenAI

  constructor() {
    super('SQLGenerationAgent')
   ...
   * @param relevantDocs - Array of relevant schema documents
   * @param model - Optional model name (defaults to env var or gpt-4o)
   * @returns SQLGenResult with SQL query and token usage
   */
  async execute(
    query: string,
    relevantDocs: string[],
...
SQL Query:`
      ...
      const usage: TokenUsage = completion.usage ? {
        ...
        total: (completion.usage.total_tokens ?? 0) || ((completion.usage.prompt_tokens ?? 0) + (completion.usage.completion_tokens ?? 0)),
        source: 'measured'
      } : {
        prompt: Math.ceil(prompt.length / 4),
        completion: Math.ceil(sql.length / 4),
        total: Math.ceil(prompt.length / 4) + Math.ceil(sql.length / 4),
        source: 'estimated'
      }

      return { sql, usage }
    } catch (error) {
      console.error(`[${this.agentName}] Error generating SQL query:`, error)
  ...
  async generateSQLQuery(
    ...
    return this.execute(query, relevantDocs, model)
  }
}
