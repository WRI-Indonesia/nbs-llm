...
/**
 * Summarization Agent
 * Generates final human-readable answers combining SQL results, context documents, and user query
 * Uses SeaLLM or other LLM endpoints for natural language generation
 */
export class SummarizationAgent extends BaseAgent {
  ...
  /**
   * Generates answer combining query, data, and context
   * @param userQuery - The user's original query
   * @param data - SQL query results (array of data rows)
   * @param context - Context documents from RAG
   * @param endpoint - Optional LLM endpoint URL
   * @param model - Optional model name
   * @returns SummarizationResult with answer text and token usage
   */
  async execute(
    userQuery: string,
    data: any[] = [],
    ...
    try {
      ...
      const limitedData = (data || []).slice(0, 5).map((row) => {
        ...
        return this.truncateToTokens(rowStr, 200) // ~200 tokens per row
      })
      ...
      const limitedContext = (context || []).slice(0, 3).map((doc) => {
        ...
        return this.truncateToTokens(docStr, 500)
      })
...
`.trim()
      ...
      if (result?.usage?.prompt_tokens || result?.usage?.completion_tokens || result?.usage?.total_tokens) {
        const prompt_tokens = result.usage.prompt_tokens ?? this.estimateTokens(systemPrompt + userMessage)
        const completion_tokens = result.usage.completion_tokens ?? this.estimateTokens(answer)
        ...
        const promptEst = this.estimateTokens(systemPrompt + userMessage)
        const completionEst = this.estimateTokens(answer)
        usage = {
          prompt: promptEst,
      ...
      console.error(`[${this.agentName}] Error generating answer:`, error)
      return {
  ...
  async generateAnswer(
    ...
    return this.execute(userQuery, data, context, endpoint, model)
  }
}
